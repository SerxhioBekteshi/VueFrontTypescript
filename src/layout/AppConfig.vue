...
export default defineComponent({
  name: "AppConfig",
  components: { InputSwitch, Button, Sidebar, SelectButton },
  props: {
    simple: {
      type: Boolean,
      default: false,
  ...
  setup() {
    ...
    const inputStyle = computed(
      () => $primevue.config.inputStyle || "outlined"
    );
    ...
    const onFocusRingColorChange = (value) => {
      ...
      if (value) {
        ...
        if (layoutConfig.darkTheme.value)
          ...
          root.style.setProperty("--p-focus-ring-color", "var(--surface-900)");
      }
    };
    return {
      onConfigButtonClick,
      onFocusRingColorChange,
